...
CREATE OR REPLACE FUNCTION public.get_public_queue()
 RETURNS TABLE(id uuid, ticket_number text, status text, priority text, created_at timestamp with time zone, called_at timestamp with time zone, service_id uuid, barber_id uuid, customer_name_masked text, service_name text, barber_name text, barber_whatsapp text, group_id uuid, companion_name text)
 LANGUAGE sql
