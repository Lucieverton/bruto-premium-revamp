...
export const Hero = () => {
  const { src: heroImage, resolved: heroResolved } = useSiteImageSlot("hero_desktop", defaultHeroImage);
  const { src: heroMobileImage, resolved: heroMobileResolved } = useSiteImageSlot("hero_mobile", defaultHeroMobileImage);
  return <header className="relative h-[100svh] md:h-[calc(100vh-100px)] w-full flex items-center justify-center overflow-hidden bg-black">
      ...
      <div className="absolute inset-0 md:hidden transition-opacity duration-300" style={{
      backgroundImage: heroMobileResolved ? `url(${heroMobileImage})` : undefined,
      opacity: heroMobileResolved ? 1 : 0,
      ...
      <div className="hidden md:block absolute inset-0 bg-no-repeat bg-center transition-opacity duration-300" style={{
      backgroundImage: heroResolved ? `url(${heroImage})` : undefined,
      ...
      <div className="hidden md:block absolute inset-0 bg-no-repeat transition-opacity duration-300" style={{
      backgroundImage: heroResolved ? `url(${heroImage})` : undefined,
      opacity: heroResolved ? 1 : 0,
